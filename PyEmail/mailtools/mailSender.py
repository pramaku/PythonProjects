...
class MailSender(MailTool):
    ...
    a nonauthenticating client: see MailSenderAuth if login required to e-mail server
    """
    ...
    def sendMessage(self, From, To, Subj, extrahdrs, bodytext, attaches,
        ...
        arguments:
        bodytext - text part of the e-mail (assumes it is already in desired encoding.)
...
class MailSenderAuth(MailSender):
    ...
    def getPassword(self):
        if not self.smtpPassword:
            ...
            except: #if not from file, force to use by Sub console class
